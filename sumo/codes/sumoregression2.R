...
df0 <- read.csv("https://spreadsheets.google.com/pub?key=0AlBuJgqcP5f3dElpb0lWcDRjZldkMzE1LW5aY1VtMHc&hl=en&single=true&gid=0&output=csv", header=FALSE)
...
for (i in 1:length(byear)){
    ...
    #We don't have Jyuryo data, so we just neglect　them as opponent data.
    if(length(bubvs) != 0) { 
